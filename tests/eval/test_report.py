...
def test_report_writer_serializes_profile_suite_metrics(tmp_path) -> None:
    from app.eval.report import write_report
    from app.eval.schemas import EvalReport, EvalResult, EvalSuiteSummary
    ...
    report = EvalReport(
        ...
        suite_id=None,
        profile_id="pr_acceptance",
        selected_suites=["profile_dag_contract", "profile_memory_snapshot"],
        case_file="memory://profile",
        strict=False,
        overall_status="PASS",
        runner_status="completed",
        total_cases=2,
        ...
        suite_summaries=[
            EvalSuiteSummary(
                ...
                failed_cases=0,
                score=1.0,
                metrics={"profile_dag_contract_pass_rate": 1.0},
            ),
            EvalSuiteSummary(
                ...
                failed_cases=0,
                score=1.0,
                metrics={"profile_memory_snapshot_pass_rate": 1.0},
            ),
        ],
        suite_metrics={
            "profile_dag_contract": {"profile_dag_contract_pass_rate": 1.0},
            "profile_memory_snapshot": {"profile_memory_snapshot_pass_rate": 1.0},
        },
        results=[
            EvalResult(
                case_id="profile_dag_case_1",
                suite="profile_dag_contract",
                status="PASS",
                passed=True,
                warnings=[],
                failures=[],
            ...
            EvalResult(
                case_id="profile_snapshot_case_1",
                suite="profile_memory_snapshot",
                status="PASS",
                passed=True,
                warnings=["evidence status preserved"],
                failures=[],
            ),
        ],
    )
    ...
    payload = written.report.model_dump(mode="json")
    markdown = written.markdown_path.read_text(encoding="utf-8")
    assert payload["suite_metrics"]["profile_dag_contract"]["profile_dag_contract_pass_rate"] == 1.0
    assert payload["suite_metrics"]["profile_memory_snapshot"]["profile_memory_snapshot_pass_rate"] == 1.0
    assert "profile_dag_contract_pass_rate" in markdown
    assert "profile_memory_snapshot_pass_rate" in markdown
    assert "warning: evidence status preserved" in markdown
