...
面向内部研发同学的项目主入口文档。本文档不是一句话项目介绍，也不是单纯的“新人导读”，而是本仓库的厚文档型研发手册：帮助你快速理解系统全貌、主要模块、关键主链路、当前实现状态、运行方式、测试路径、边界约束和当前稳定结论。
...
## M7 Minimum Commercial Production Readiness

当前主线状态：

- `M7 completed`
- `M8 not started`
- 当前仓库按已交付最终版维护，不启动新的后续实现阶段

M7 交付的是最小商业运行边界，而不是 full enterprise production certification。当前交付边界包括：

- Docker / local startup readiness
- backup / restore / rollback readiness
- monitoring / alerting / audit boundary
- release gate / load smoke readiness
- final acceptance review and operator handoff docs

M7 commercial handoff 入口：
...
Canonical M7D production release gate:

```bash
python -m app.eval.runner --profile production_release --strict
```

`python -m app.release.pre_m3_gate --profile production_release --strict` 仅保留为 legacy / compatibility reference。

当前文档阅读口径：

- `README.md` 负责给出最新主线结论和稳定入口
- `PLANNING.md` 负责记录当前真相与历史阶段快照
- 历史 `docs/plans/`、`docs/reviews/`、`docs/runbooks/` 保留阶段性语境，不代表新的后续开发承诺
...
`app/release/` 仍然保留历史兼容的 Pre-M3 release gate 入口：
...
它不是测试替代品，而是历史结构化发布决策入口。M7 commercial readiness 的 canonical gate 以 shared eval `app.eval.runner` 为准。

Pre-M3 gate 的状态语义仍然是：
...
- M6C 已补 rollout / trace / metrics / fallback observability，但 semantic context injection 仍默认关闭
- M6C full trace 只存在于 shared memory runtime metadata；sanitized summary 只存在于 `execution_trace.internal_metadata`
- public session API 会过滤 `_internal*` handoff 字段；M6C 不做 return-object refactor，也不做 DB audit stream
- 上述 M6 结论是历史分阶段口径；这些能力的最小商业交付边界已经在 M7 完成收口，但仍不等同 full enterprise platform
...
- M6C 现在增加了 rollout / observability 能力：
  - shared runtime full trace metadata
  - sanitized execution-trace summary
  - fallback / policy-block / context-budget observability
  - rollback runbook and acceptance coverage
- semantic context injection 仍然 default-off
- SQL/Data Agent semantic supplement 仍然 disabled
- full trace 仍是 metadata-only，不进入 prompt
- sanitized summary 只进入 `execution_trace.internal_metadata["semantic_memory"]`
- public session API 会过滤 `_internal*` fields
- M6C 不做 return-object refactor，也不做 DB audit stream
- 当前主线状态可表述为：`M6 completed`；`M7 completed`；`M8 not started`
...
这是 CLI，不是 HTTP API。当前需要区分 canonical 入口与 historical compatibility 入口：
...
当前默认发布门禁以 `app.eval.runner` 为准；`pre_m3_gate` 不再作为 M7 主线默认 gate。
...
当前仓库已经按最终版收口，后续不再在本仓库内启动新的默认实施阶段。

当前稳定边界：

- 主线状态固定为 `M7 completed`、`M8 not started`
- M7 的含义固定为 `Minimum Commercial Production Readiness`
- canonical release gate 固定为 `python -m app.eval.runner --profile production_release --strict`
- `app.release.pre_m3_gate` 只保留为历史兼容入口
- 现有 runbook / review / checklist / handoff 文档构成可交接、可验证、可回滚的最小商业闭环
- 本仓库当前不承诺 full enterprise production certification、full cloud DR、full observability platform、SIEM、Kubernetes / CD、full production load testing
